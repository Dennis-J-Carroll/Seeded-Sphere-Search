...
class SeededSphereSearch:
    ...
    def seeded_search(self, query, seed_doc_id, top_k=10, use_ellipsoidal=False, ellipsoidal_transformer=None, seed_weight=0.5):
        """
        Search for relevant documents given a query and a seed document
        
        Args:
            query: Text query
            seed_doc_id: ID of the seed document to influence search results
            top_k: Number of top results to return
            use_ellipsoidal: Whether to use ellipsoidal transformation for scoring
            ellipsoidal_transformer: EllipsoidalTransformation instance
            seed_weight: Weight of the seed document vs query (between 0 and 1)
        ...
        if seed_doc_id not in self.vocabulary:
            raise ValueError(f"Seed document with ID {seed_doc_id} not found")
        
        # Encode the query
        query_embedding = self._encode_query(query)
        
        # Get seed document embedding
        seed_embedding = self.refined_embeddings[seed_doc_id]
        
        # Combine query and seed embeddings
        import numpy as np
        combined_embedding = (1 - seed_weight) * query_embedding + seed_weight * seed_embedding
        combined_embedding = self._normalize_vector(combined_embedding)
        
        # Apply ellipsoidal transformation if requested
        if use_ellipsoidal and ellipsoidal_transformer is not None:
            combined_embedding = ellipsoidal_transformer.transform_query(combined_embedding)
        
        # Calculate scores for all documents
        scores = []
        for doc_id, embedding in self.refined_embeddings.items():
            # Skip the seed document itself
            ...
            # Calculate angular distance
            dot_product = np.dot(combined_embedding, embedding)
            # Clamp to avoid numerical issues
            dot_product = max(-1.0, min(1.0, dot_product))
            distance = np.arccos(dot_product)
            
            # Calculate similarity score
            similarity = np.exp(-self.config["alpha"] * distance)
            
            scores.append({
                "id": doc_id,
                "title": self.vocabulary[doc_id]["title"],
                "content": self.vocabulary[doc_id].get("content", "")[:500] if "content" in self.vocabulary[doc_id] else "",
                "tags": self.vocabulary[doc_id].get("tags", []),
    ...
    def _encode_query(self, query):
        ...
        if self.config["use_pretrained"]:
            ...
            embeddings = self._mean_pooling(outputs.last_hidden_state, attention_mask=inputs["attention_mask"])
